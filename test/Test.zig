...
pub fn set(self: *Test, comptime tag: aggregates.Tag, element: Element, value: tag.Value()) void {
    var cv_slice = self.element_tree.cascadedValuesSlice(allocator);
    const cv = cv_slice.get(element);
    cv_slice.add(cv, tag, value) catch |err| fail(err);
}

pub fn setText(self: *Test, element: Element, value: ElementTree.Text) void {
    var cv_slice = self.element_tree.cascadedValuesSlice(allocator);
    cv_slice.setText(element, value);
}

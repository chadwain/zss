...
pub fn run(tests: []const *Test, output_parent_dir: []const u8) !void {
    ...
    for (tests, 0..) |t, ti| {
        ...
        var layout = zss.Layout.init(&t.element_tree, t.root_element, allocator, t.width, t.height, &t.env, t.fonts);
        defer layout.deinit();
        ...
        for (0..pages) |i| {
            ...
            try zss.render.opengl.drawBoxTree(&renderer, t.env.images.view(), &box_tree, &draw_list, allocator, viewport);
            zgl.flush();

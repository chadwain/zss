...
pub const Document = struct {
    ...
    pub fn rootZssNode(document: *const Document) ?Environment.NodeId {
        ...
        return .{ .document = document.id, .value = root_node };
    }
    ...
    pub const tree_interface_fns = struct {
        pub fn node_edge(context: *const anyopaque, node: Environment.NodeId, edge: Environment.TreeInterface.Edge) ?Environment.NodeId {
            const document: *const Document = @alignCast(@ptrCast(context));
            assert(node.document == document.id);
            const index: Node.Index = @intCast(node.value);

            const result_index: Node.Index = sw: switch (edge) {
                .parent => {
                    const parent = document.tree.items(.parent)[index];
                    if (parent == index) return null;
                    break :sw parent;
                },
                inline .previous_sibling, .next_sibling => |comptime_which| {
                    const sibling = document.tree.items(std.enums.nameCast(NodeTree.Field, comptime_which))[index];
                    if (sibling == 0) return null;
                    break :sw sibling;
                },
                inline .first_child, .last_child => |comptime_which| {
                    const last_child = document.tree.items(.last_child)[index];
            ...
            return .{ .document = document.id, .value = result_index };
        }
    };
};
...
pub fn createDocument(
    ...
    const ns = struct {
        ...
        fn finalize(
            tr: *const Document.NodeTree.Slice,
            tree_index: Document.Node.Index,
            last_child: Document.Node.Index,
        ) void {
    ...
    const root_tree_index, const root_zml_children_index =
    ...
    while (stack.top) |*top| {
        ...
        const tree_index, const zml_children_index =
...
) !struct { Document.Node.Index, ?Ast.Index } {
    assert(zml_node_index.tag(ast) == .zml_node);
    const tree_index = std.math.cast(Document.Node.Index, try tree.addOne(allocator)) orelse return error.Overflow;
    const zss_node_id: Environment.NodeId = .{ .document = document.id, .value = tree_index };
...
test "create a zml document" {
    ...
    {
        const node = document.rootZssNode() orelse return error.TestFailure;
        const cascaded_values = env.getNodeProperty(.cascaded_values, node);
    ...
    {
        const node = document.rootZssNode().?.firstChild(&env) orelse return error.TestFailure;
        try std.testing.expectEqual(type1, env.getNodeProperty(.type, node).name);
        const cascaded_values = env.getNodeProperty(.cascaded_values, node);
    ...
    {
        const node = document.rootZssNode().?.lastChild(&env) orelse return error.TestFailure;
        try std.testing.expectEqual(type2, env.getNodeProperty(.type, node).name);
        const cascaded_values = env.getNodeProperty(.cascaded_values, node);
        const all = cascaded_values.all orelse return error.TestFailure;
